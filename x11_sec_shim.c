                                    ...
                                    enum x11_sec_reason *out_reason)
{
    ...
    if (context->audit == NULL) return X11_SEC_DENY;
    ...
    (void)snprintf(record.detail, sizeof(record.detail), "%s", x11_sec_reason_string(reason));
    if (context->audit(&record, context->audit_context) != 0 &&
        context->policy.fail_closed_on_audit_error && out_reason != NULL)
...
void x11_sec_policy_default(x11_sec_policy *policy)
{
    ...
    policy->allowed_operations = X11_SEC_OP_READ_PROPERTIES | X11_SEC_OP_SELECT_INPUT;
    policy->requests_per_window = X11_SEC_DEFAULT_REQUEST_LIMIT;
    policy->window_ns = X11_SEC_DEFAULT_WINDOW_NS;
...
int x11_sec_init(x11_sec_context *context, const x11_sec_policy *policy,
                 const uint8_t *expected_token, size_t expected_token_length,
                 x11_sec_audit_fn audit, void *audit_context)
{
    if (context == NULL || policy == NULL || expected_token_length > X11_SEC_MAX_TOKEN_BYTES ||
        (expected_token_length != 0u && expected_token == NULL)) return -1;
    if (policy->window_ns == 0u || policy->requests_per_window == 0u) return -1;
    memset(context, 0, sizeof(*context));
    context->policy = *policy;
    context->expected_token_length = expected_token_length;
    if (expected_token_length != 0u) memcpy(context->expected_token, expected_token, expected_token_length);
    context->audit = audit;
...
enum x11_sec_decision x11_sec_authorize(x11_sec_context *context,
                                         ...
                                         enum x11_sec_reason *reason)
{
    if (reason != NULL) *reason = X11_SEC_OK;
    if (context == NULL || client == NULL || request == NULL ||
        ...
        if (reason != NULL) *reason = X11_SEC_BAD_ARGUMENT;
        return X11_SEC_DENY;
    }
    if (context->policy.allowed_operations == 0u)
        return reject(context, client, request, now_ns, X11_SEC_POLICY_DISABLED, reason);
    if (request->operation == 0u ||
        (request->operation & ~context->policy.allowed_operations) != 0u)
        return reject(context, client, request, now_ns, X11_SEC_OPERATION_DENIED, reason);
    if (request->payload_length > X11_SEC_MAX_PAYLOAD_BYTES ||
        request->resource_count > X11_SEC_MAX_RESOURCES_PER_REQUEST ||
        request->has_display == 0u || request->has_resource == 0u)
        return reject(context, client, request, now_ns, X11_SEC_MALFORMED_REQUEST, reason);
    if ((client->unix_socket == 0u && client->loopback == 0u) ||
        ...
        return reject(context, client, request, now_ns, X11_SEC_UNTRUSTED_ORIGIN, reason);
    if (context->policy.require_authentication && !client->authenticated)
        return reject(context, client, request, now_ns, X11_SEC_NOT_AUTHENTICATED, reason);
    if ((client->token_length != 0u && client->token == NULL) ||
        !secure_equal(client->token, client->token_length,
                      context->expected_token, context->expected_token_length))
        return reject(context, client, request, now_ns, X11_SEC_BAD_TOKEN, reason);
    ...
    if (context->requests_in_window >= context->policy.requests_per_window)
        return reject(context, client, request, now_ns, X11_SEC_RATE_LIMITED, reason);
